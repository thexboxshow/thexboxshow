		...
		foreach( $this->form_options as $id => $field ) {
			...
			elseif( isset( $instance['panels_info'] ) && isset( $field['type'] ) ) {
				switch ( $field['type'] ) :
					...
					case 'select':      foreach ( $defaults[ $field['options'] ] as $sopk => $sopv ) { $defaults[ $id ] = $sopk; break; } break;
					case 'multiselect': $defaults[ $id ] = array(); break;
					case 'smallselect': foreach ( $defaults[ $field['options'] ] as $sopk => $sopv ) { $defaults[ $id ] = $sopk; break; } break;
					case 'radio':
					case 'images':      foreach ( $defaults[ $field['options'] ] as $sopk => $sopv ) { $defaults[ $id ] = $sopk; break; } break;
